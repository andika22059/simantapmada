        ...
        <form @submit.prevent="handleSubmit">
          ...
          <div class="form-actions">
            <button type="submit" class="btn-save">
              <i class="fa-solid fa-save"></i> Simpan Surat Masuk
            </button>
          </div>
        </form>
      </div>
...
const handleSubmit = async () => {
  if (!form.value.file) {
    Swal.fire("Peringatan!", "File dokumen surat wajib diupload.", "warning");
    return;
  }
  ...
  fd.append("disposisi_ke", form.value.disposisi_ke || "-");
  ...
  fd.append("simpan_arsip", form.value.simpan_arsip ? "1" : "0");

  try {
    Swal.fire({
      title: "Menyimpan Arsip...",
      text: "Sedang Mengunggah Dokumen ke Gudang Arsip...",
      allowOutsideClick: false,
      didOpen: () => Swal.showLoading(),
    });

    await axios.post(`${import.meta.env.VITE_API_URL}/api/surat-masuk`, fd, {
    ...
    Swal.fire(
      ...
      `Surat masuk berhasil diarsipkan.${
        form.value.simpan_arsip ? " Surat ini juga tersimpan di E-Arsip." : ""
      }`,
      "success",
    ).then(() => {
    ...
    console.error(error);
    Swal.fire("Gagal", "Terjadi kesalahan saat mengunggah surat .", "error");
  }
};
</script>
